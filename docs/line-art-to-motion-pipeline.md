...
The renderer and page are the only hand-written parts. Copy `src/primitives/DroneSearch404.mjs` + `promo/drone-404.html` (traced art), or the flipbook renderer pattern in §1b, and reuse the shared timeline, flight controller and hover layer unchanged, scaled by one unit factor.

...
Một phiên khác đã thay package.json/README/tsconfig trong lúc setup. Các npm commands bên dưới thuộc [manifest đề xuất](../plans/toolchain-package.proposed.json), **chưa được hợp nhất** vào package đang chạy. Xem [entry point bổ sung](architecture-supplement.md). Các file src/scripts/schema/playground đã tồn tại; Node-only scripts chạy trực tiếp được.

...
3. `scripts/build-drone-404-geometry.mjs` — assigns traced points to parts (4 blade sets, lens),
...
7. `promo/drone-404.html` — the page. `scripts/render-drone-404-deliverable.mjs` — proofs, MP4, GIF.
...
- 3.1 s lift-off = damped step (overshoot, settle). 4.0 s -> mission, period 10 s:
...
- Tuning: attitude zeta 0.68, position 0.78, gimbal 0.7 -> one counter-swing (~4 deg) then level in
  ~0.3 s; hover wobble +-0.7 deg; max bank 12 deg; gust peak 7 deg. Evidence: `flight-bank-profile.png`.
...
- Evidence: `interaction-contact-sheet.png` + logged drone transforms (rest 1.6 px -> pushed 48.7 px, level).

## Acceptance (evidence in `promo/drone-404-proofs/`)
- [x] Trace vs reference: 0.368% pixels differ (fuzz 30%) — `fidelity-overlay.png`, `fidelity-metric.txt`
- [x] `npm test` 60/60, `npx tsc --noEmit`, `python3 scripts/anti-flop-gate.py` exit 0
- [x] Motion IR valid (`node scripts/validate-ir.mjs fixtures/drone-search-404.motion.json`)
- [x] `ui gate` PASS, axe 0 violations x3, render probe clean at 375/768/1440
